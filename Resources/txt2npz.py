...
def txt2npz(data_name, is_transform_y=False, is_remove_empty=False):
    filename = home_dir + 'Resources/' + data_name
    ...
    if is_transform_y:
        ...
        # y_vec = (y_vec * 2) - 3
        # pass
        y_vec = (y_vec - 1).astype(int)
        # y_vec = ((y_vec + 1) / 2).astype(int)
    print(y_vec[:100])
    ...
    out_filename = home_dir + 'Resources/' + data_name + '.npz'
    numpy.savez(out_filename, data_name=data_name, x_mat=x_mat, y_vec=y_vec)


if __name__ == '__main__':
    data_name = 'covtype'
    txt2npz(data_name, is_transform_y=True)
